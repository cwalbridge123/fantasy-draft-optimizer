...
with st.sidebar:
    ...
    if run_btn:
        # Clear previous draft and AI summary when re-optimizing
...
if not st.session_state.get("draft_done") and not run_btn:
    # ── Welcome state ──
    ...
    # ── Run optimizer ──
    # Only rerun the optimizer if the button was just clicked
    if run_btn:
        ...
        if success and not drafted.empty:
        ...
        drafted = st.session_state.get("drafted", None)
        picks   = st.session_state.get("picks",   [])
        taken   = st.session_state.get("taken",   {})
        success = drafted is not None and not drafted.empty
