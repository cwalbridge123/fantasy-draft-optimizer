        ...
        if st.button("✨ Generate AI Summary", type="primary"):
            with st.spinner("Claude is breaking down your draft..."):
                try:
                    ...
                    # Always show HTTP status so we know if request reached API
                    ...
                    if "error" in data:
                        st.error(f"API error ({data['error'].get('type','?')}): {data['error'].get('message', data['error'])}")
                        st.stop()
                    ...
                    st.error("Request timed out after 60s — try again.")
                except Exception as e:
                    st.error(f"Something went wrong: {type(e).__name__}: {e}")
                    st.warning("Showing auto-generated summary instead.")
